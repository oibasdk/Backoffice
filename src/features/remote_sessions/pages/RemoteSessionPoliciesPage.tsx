...
import { createRemoteSessionPolicy, listRemoteSessionPolicies } from "../api";

export const RemoteSessionPoliciesPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["remote-session-policies", queryParams, tokens?.accessToken],
    queryFn: () => listRemoteSessionPolicies(tokens?.accessToken || "", queryParams),
    enabled: Boolean(tokens?.accessToken),
  });

  const createMutation = useMutation({
    mutationFn: () =>
      createRemoteSessionPolicy(tokens?.accessToken || "", {
        name,
    ...
    onSuccess: () => {
      pushToast({ message: t("remote_sessions.policy.created"), severity: "success" });
      queryClient.invalidateQueries({ queryKey: ["remote-session-policies"] });
      setDialogOpen(false);
      setName("");
      setDescription("");
      setScopeType("global");
  ...
  const columns = [
    { key: "name", label: t("remote_sessions.policy.name") },
    { key: "scope", label: t("remote_sessions.policy.scope") },
  ...
  return (
    <PermissionGate permissions={["remote_session_policy.view"]}>
      <Stack spacing={3}>
        <PageHeader
          title={t("remote_sessions.policy.title")}
          subtitle={t("remote_sessions.policy.subtitle")}
          actions={
            <PermissionGate permissions={["remote_session_policy.create"]}>
        ...
        <FilterBar
          savedViews={{
            storageKey: "remote_session.policies",
        ...
        >
          ...
          <TextField
            select
            label={t("remote_sessions.policy.scope_type")}
            value={scopeFilter}
        ...
        <DataTable
          ...
          exportFilename="remote_session_policies.csv"
          ...
          onRowClick={(row) => navigate(`/remote-session-policies/${row.id}`)}
        />
      </Stack>

      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{t("remote_sessions.policy.new")}</DialogTitle>
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              label={t("remote_sessions.policy.name")}
              value={name}
              onChange={(event) => setName(event.target.value)}
            />
            <TextField
              label={t("remote_sessions.policy.description")}
              value={description}
            ...
            {scopeType !== "global" && (
              <TextField
                label={t("remote_sessions.policy.scope_value")}
                value={scopeValue}
