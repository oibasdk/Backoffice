...
import { createEscalationPolicy, listEscalationPolicies } from "../api";

export const EscalationPoliciesPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["escalation-policies", queryParams, tokens?.accessToken],
    queryFn: () => listEscalationPolicies(tokens?.accessToken || "", queryParams),
    enabled: Boolean(tokens?.accessToken),
  });

  const createMutation = useMutation({
    mutationFn: () =>
      createEscalationPolicy(tokens?.accessToken || "", {
        name,
    ...
    onSuccess: () => {
      pushToast({ message: t("escalation.policy.created"), severity: "success" });
  ...
  const columns = [
    { key: "name", label: t("escalation.policy.name") },
    { key: "scope", label: t("escalation.policy.scope") },
  ...
  return (
    <PermissionGate permissions={["escalation_policy.view"]}>
      <Stack spacing={3}>
        <PageHeader
          title={t("escalation.policy.title")}
          subtitle={t("escalation.policy.subtitle")}
          actions={
            <PermissionGate permissions={["escalation_policy.create"]}>
        ...
        <FilterBar
          savedViews={{
            storageKey: "escalation.policies",
        ...
        >
          ...
          <TextField
            select
            label={t("escalation.policy.scope_type")}
            value={scopeFilter}
        ...
        <DataTable
          ...
          exportFilename="escalation_policies.csv"
          ...
          onRowClick={(row) => navigate(`/escalation-policies/${row.id}`)}
        />
      </Stack>

      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{t("escalation.policy.new")}</DialogTitle>
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              label={t("escalation.policy.name")}
              value={name}
              onChange={(event) => setName(event.target.value)}
            />
            <TextField
              label={t("escalation.policy.description")}
              value={description}
            ...
            {scopeType !== "global" && (
              <TextField
                label={t("escalation.policy.scope_value")}
                value={scopeValue}
