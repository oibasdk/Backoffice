...
import { createSlaPolicy, listSlaPolicies } from "../api";
...
export const SlaPoliciesPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["sla-policies", queryParams, tokens?.accessToken],
    queryFn: () => listSlaPolicies(tokens?.accessToken || "", queryParams),
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createSlaPolicy(tokens?.accessToken || "", {
        name,
    ...
    onSuccess: () => {
      pushToast({ message: t("sla.policy.created"), severity: "success" });
  ...
  const columns = [
    { key: "name", label: t("sla.policy.name") },
    { key: "scope", label: t("sla.policy.scope") },
  ...
  return (
    <PermissionGate permissions={["sla_policy.view"]}>
      <Stack spacing={3}>
        <PageHeader
          title={t("sla.policy.title")}
          subtitle={t("sla.policy.subtitle")}
          actions={
            <Stack spacing={1} alignItems={{ xs: "flex-start", sm: "flex-end" }}>
              <PermissionGate permissions={["sla_policy.create"]}>
        ...
        <DataTable
          ...
          exportFilename="sla_policies.csv"
          ...
          onRowClick={(row) => navigate(`/sla-policies/${row.id}`)}
        />
      </Stack>

      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{t("sla.policy.new")}</DialogTitle>
        <DialogContent>
          <Stack spacing={2} mt={1}>
            <TextField
              label={t("sla.policy.name")}
              value={name}
              onChange={(event) => setName(event.target.value)}
            />
            <TextField
              label={t("sla.policy.description")}
              value={description}
            ...
            {scopeType !== "global" && (
              <TextField
                label={t("sla.policy.scope_value")}
                value={scopeValue}
